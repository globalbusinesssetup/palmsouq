...
class UsersController extends ControllerHelper
{
    ...
    public function all(Request $request)
    {
        try {
            ...
            return response()->json(new Response($request->token, $data), 200);
    ...
    public function login(Request $request)
    {
        try {
            ...
            if ($request->input('token')) {
                ...
                if ($user) {
                    return response()->json(new Response($request->input('token'), $user));
                }
            }
            ...
            if (!$user) {
                return response()->json(Validation::error(null,
                    __('lang.invalid_credentials', [], $lang)
                ));
            }

            $password_check = Validation::password_check($user, $request->input('password'));
            if ($password_check) {
            ...
            if ($request->input('remember_token')) {
                $data['expires_in'] = Carbon::now()->addYear(1);
            } else {
                $data['expires_in'] = Carbon::now()->addMonth(1);
            }
            ...
            if ($user->user_type === 'corporate') {
                $userArr['company_name'] = $user->company_name;
                $userArr['trade_license'] = $user->trade_license;
                $userArr['trn_certificate'] = $user->trn_certificate;
            }

            if ($request->input('user_token')) {
                GuestUser::where('user_token', $request->input('user_token'))->update([
                    'name' => $user->name,
                    'email' => $user->email
                ]);

                Cart::where('user_id', null)
                    ->where('user_token', $request->input('user_token'))
            ...
            return response()->json(new Response($request->input('token'), $data));

        } catch (\Exception $ex) {
            return response()->json(Validation::error($request->input('token'), $ex->getMessage()));
        }
    }
